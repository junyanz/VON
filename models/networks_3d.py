...
def _calc_grad_penalty(netD, real_data, fake_data, device, type='mixed', norm=1.0, ll=10.0):
    if ll > 0.0:
        if type == 'real':
        ...
        gradient_penalty = (((gradients + 1e-16).norm(2, dim=1) - norm) ** 2).mean() * ll        # added eps
