...
#define THC_GENERIC_FILE "generic/vtn_cuda_kernel_generic.cu"
#else
...
static inline void VTN_(SpatialGridSamplerBilinear_shapeCheck)(
    THCState *state,
    THCTensor *inputTensor,
    THCTensor *grid,
    THCTensor *gradOutput) {
  THCUNN_argCheck(state, THCTensor_(nDimension)(state, inputTensor) == 5, 2, inputTensor,
  ...
  int nbatch   = THCTensor_(size)(state, inputTensor, 0);
  int channels = THCTensor_(size)(state, inputTensor, 1);
  // int isz1   = THCTensor_(size)(state, inputTensor, 2);
  // int isz2    = THCTensor_(size)(state, inputTensor, 3);
  // int isz3    = THCTensor_(size)(state, inputTensor, 4);
  int osz1   = THCTensor_(size)(state, grid, 1);
  int osz2   = THCTensor_(size)(state, grid, 2);
  int osz3   = THCTensor_(size)(state, grid, 3);
...
__global__ void VTN_(BilinearSampler3DChannelFirst_updateOutput_kernel)(
    real* inputTensor, // N * C * isz1 * isz2 * isz3
    int N,
    int C,
    int isz1, int isz2, int isz3,
    int isdb, int isdc, int isd1, int isd2, int isd3,
    real* grid,        // N * osz1 * osz2 * osz3 * 3
    int gsdb, int gsd1, int gsd2, int gsd3, int gsd4,
    real* output,      // N * C * osz1 * osz2 * osz3
...
int VTN_(BilinearSampler3DChannelFirst_updateOutput_wrap)(THCState* state, THCTensor* inputTensor, THCTensor* grid, THCTensor* output) {
  ...
  VTN_(SpatialGridSamplerBilinear_shapeCheck)(state, inputTensor, grid, output);

  // call kernel function
  int N = THCTensor_(size)(state, inputTensor, 0);
  int C = THCTensor_(size)(state, inputTensor, 1);
  int isz1 = THCTensor_(size)(state, inputTensor, 2);
  int isz2 = THCTensor_(size)(state, inputTensor, 3);
  int isz3 = THCTensor_(size)(state, inputTensor, 4);
  int osz1 = THCTensor_(size)(state, grid, 1);
  int osz2 = THCTensor_(size)(state, grid, 2);
  int osz3 = THCTensor_(size)(state, grid, 3);
  THCTensor_(resize5d)(state, output, N, C, osz1, osz2, osz3);

  int count = (N * osz1 * osz2 * osz3);
  VTN_(BilinearSampler3DChannelFirst_updateOutput_kernel)
    <<<GET_BLOCKS(count), CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state)>>>(
      THCTensor_(data)(state, inputTensor),
      N, C, isz1, isz2, isz3,
      THCTensor_(stride)(state, inputTensor, 0),
      THCTensor_(stride)(state, inputTensor, 1),
      THCTensor_(stride)(state, inputTensor, 2),
      THCTensor_(stride)(state, inputTensor, 3),
      THCTensor_(stride)(state, inputTensor, 4),
      THCTensor_(data)(state, grid),
      THCTensor_(stride)(state, grid, 0),
      THCTensor_(stride)(state, grid, 1),
      THCTensor_(stride)(state, grid, 2),
      THCTensor_(stride)(state, grid, 3),
      THCTensor_(stride)(state, grid, 4),
      THCTensor_(data)(state, output),
      osz1, osz2, osz3,
      THCTensor_(stride)(state, output, 0),
      THCTensor_(stride)(state, output, 1),
      THCTensor_(stride)(state, output, 2),
      THCTensor_(stride)(state, output, 3),
      THCTensor_(stride)(state, output, 4),
      count);
...
__launch_bounds__(CUDA_NUM_THREADS)
__global__ void VTN_(BilinearSampler3DChannelFirst_updateGradInput_kernel)(
    real* inputTensor, // N * C * isz1 * isz2 * isz3
    int N,
    int C,
    int isz1, int isz2, int isz3,
    ...
    real* grid,        // N * osz1 * osz2 * osz3 * 3
    int gsdb, int gsd1, int gsd2, int gsd3, int gsd4,
    real* gradInput,   // N * C * isz1 * isz2 * isz3
    int gisdb, int gisdc, int gisd1, int gisd2, int gisd3,
    real* gradGrid,    // N * osz1 * osz2 * osz3 * 3
    int ggsdb, int ggsd1, int ggsd2, int ggsd3, int ggsd4,
    real* gradOutput,  // N * C * osz1 * osz2 * osz3
    int osz1, int osz2, int osz3,
...
int VTN_(BilinearSampler3DChannelFirst_updateGradInput_wrap)(THCState* state, THCTensor* inputTensor, THCTensor* grid, THCTensor* gradInput, THCTensor* gradGrid, THCTensor* gradOutput) {
  ...
  VTN_(SpatialGridSamplerBilinear_shapeCheck)(state, inputTensor, grid, gradOutput);
  ...
  int osz2 = THCTensor_(size)(state, grid, 2);
  int osz3 = THCTensor_(size)(state, grid, 3);
  THCTensor_(resize5d)(state, gradInput, N, C, isz1, isz2, isz3);
  THCTensor_(resize5d)(state, gradGrid, N, osz1, osz2, osz3, 3);
  THCTensor_(zero)(state, gradInput);
  THCTensor_(zero)(state, gradGrid);

  int count = (N * osz1 * osz2 * osz3);
  VTN_(BilinearSampler3DChannelFirst_updateGradInput_kernel)
    <<<GET_BLOCKS(count), CUDA_NUM_THREADS, 0, THCState_getCurrentStream(state)>>>(
      THCTensor_(data)(state, inputTensor),
      N, C, isz1, isz2, isz3,
      THCTensor_(stride)(state, inputTensor, 0),
      ...
      THCTensor_(stride)(state, gradInput, 0),
      THCTensor_(stride)(state, gradInput, 1),
      THCTensor_(stride)(state, gradInput, 2),
      THCTensor_(stride)(state, gradInput, 3),
      THCTensor_(stride)(state, gradInput, 4),
      THCTensor_(data)(state, gradGrid),
      THCTensor_(stride)(state, gradGrid, 0),
      THCTensor_(stride)(state, gradGrid, 1),
      THCTensor_(stride)(state, gradGrid, 2),
      THCTensor_(stride)(state, gradGrid, 3),
      THCTensor_(stride)(state, gradGrid, 4),
      THCTensor_(data)(state, gradOutput),
      osz1, osz2, osz3,
      THCTensor_(stride)(state, gradOutput, 0),
      THCTensor_(stride)(state, gradOutput, 1),
      THCTensor_(stride)(state, gradOutput, 2),
      THCTensor_(stride)(state, gradOutput, 3),
      THCTensor_(stride)(state, gradOutput, 4),
      count);
